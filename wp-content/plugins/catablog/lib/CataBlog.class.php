...
class CataBlog {
	...
	**********************************************/
	...
	public function frontend_content($atts) {
		...
		foreach ($results as $result) {
			...
			$values['languages']    = $result->languages;
	...
	**********************************************/
	...
	private function save_item() {
		...
		$langs = trim($escaped_post['languages']);
		...
		if (isset($_REQUEST['id'])) {
			...
			$id = $_REQUEST['id'];
			if ($new_image) {
				$wpdb->update($table, array('image'=>$image_name, 'title'=>$title, 'link'=>$link, 'description'=>$desc, 'tags'=>$tags, 'price'=>$price, 'product_code'=>$code, 'languages'=>$langs), array('id'=>$id), array('%s', '%s', '%s', '%s', '%s', '%f', '%s'), array('%d'));
			}
			else {
				$wpdb->update($table, array('title'=>$title, 'link'=>$link, 'description'=>$desc, 'tags'=>$tags, 'price'=>$price, 'product_code'=>$code, 'languages' => $langs), array('id'=>$id), array('%s', '%s', '%s', '%s', '%f', '%s'), array('%d'));
			}
			...
			$wpdb->insert($table, array('ordinal'=>0, 'image'=>$image_name, 'title'=>$title, 'link'=>$link, 'description'=>$desc, 'tags'=>$tags, 'price'=>$price, 'product_code'=>$code, 'languages' => $langs), array('%d', '%s', '%s', '%s', '%s', '%s', '%f', '%s'));
	...
	private function load_xml_to_database($xml) {
		...
		foreach ($xml->item as $item) {
			...
			$row['languages'] = (string) $item->languages;
